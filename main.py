...
def load_assets():
    ...
    # --- HÌNH NỀN GIAO DIỆN LEVEL (ĐỘNG HOẶC TĨNH) ---
    bg3_frames = []
    frame_idx = 1
    while True:
    ...
    # --- BẢN ĐỒ ĐẤT (GROUND NỀN) THEO CHẾ ĐỘ CHƠI ---
    for i in range(1, 5):
    ...
    # --- HIỆU ỨNG ĐÁM MÂY VÀ BÓNG NẨY ---
    cloud_assets = []
    cloud_size_ranges = [
        (150, 60, 200, 80), (200, 80, 250, 100), 
        (250, 100, 300, 120), (300, 120, 350, 140), 
        (350, 140, 450, 180)
    ]
    if os.path.exists('cloud.png'):
        ...
        for min_w, min_h, max_w, max_h in cloud_size_ranges:
            ...
            cloud_img = pygame.transform.scale(cloud_base, (w, h))
    ...
    # --- KHUNG PREVIEW MÀN CHƠI ANIMATED ---
...
# Khởi tạo tài nguyên toàn cục ngay khi chạy app
...
# ==========================================
# CÁC LỚP ĐỐI TƯỢNG ĐỒ HỌA TRANG TRÍ (CLASSES)
# ==========================================
class Cloud:
...
def draw_text_with_shadow(text, font, color, surface, x, y):
    # Tạo hiệu ứng đổ bóng chữ 3D chuẩn Pixel Art
...
def draw_styled_button(surface, rect, color, text, font):
    # Vẽ nút bo tròn có độ trong suốt nhẹ (Alpha block)
...
def transition_to_black_pixel(clock):
    # Hiệu ứng thu tròn cảnh màn hình dạng Pixel cổ điển
    pixel_size = 16 
...
def transition_fade_in(clock, draw_func):
    # Hiệu ứng mờ dần (Fade in) từ màn hình đen mượt mà
...
# ==========================================
# GIAO DIỆN CHỌN MÀN CHƠI (STAGE SELECTION)
# ==========================================
def level_selection_menu(clock):
    ...
    def draw_screen(mx=-100, my=-100):
        ...
        # Vẽ các thành phần trang trí (Logic update đã được tách ra vòng loop ngoài)
        for cloud in clouds: 
            cloud.draw(screen)
        ...
        # Render các ô Preview chế độ chơi
        for i, rect in enumerate(frames_base):
            is_hover = rect.collidepoint((mx, my))
            frame_rect = rect.inflate(12, 12)
            
            # Khung viền gỗ/vàng Pixel đặc trưng bao quanh Card preview
            ...
            # Hiệu ứng phát sáng nhấp nháy nhẹ khi hover chuột vào Box màn chơi
            if is_hover:
                overlay = pygame.Surface(rect.size, pygame.SRCALPHA)
                overlay.fill((255, 255, 255, 60 + int(math.sin(time_now * 0.01) * 30))) 
                screen.blit(overlay, rect.topleft)

            # Hiển thị Tên chế độ chơi phía trên Card tương ứng
    ...
    while True:
        ...
        # Cập nhật chuyển động của các asset trang trí
        for cloud in clouds: 
            cloud.update()
        for ball in whiteballs: 
            ...
            for i, rect in enumerate(frames_base):
                if rect.collidepoint((mx, my)):
                    if game_assets.get('sound_selectstage'):
                        game_assets['sound_selectstage'].play()
                    
                    pygame.mixer.music.stop()  # Dừng phát nhạc nền Menu để nhường luồng audio cho Gameplay theme
                    transition_to_black_pixel(clock) 
                    
                    mode_name = hover_texts[i][0]  
                    ground_key = mode_ground_mapping[i] 
                    
                    # Gọi hàm khởi chạy chế độ chơi từ Module gameplay chính
                    gameplay.run_game_mode(screen, clock, WIDTH, HEIGHT, game_assets, transition_to_black_pixel, mode_name, ground_key)
                    
                    # Sau khi chơi xong thoát ra màn hình chọn, tải và phát lại nhạc nền Menu
                    try:
                        pygame.mixer.music.load(os.path.join('sound', 'menusound.mp3'))
...
# ==========================================
# GIAO DIỆN MENU CHÍNH (MAIN MENU LOOP)
# ==========================================
def main_menu():
    click = False
    ...
    # Định nghĩa cấu trúc Rect nút bấm cố định ngoài vòng lặp giúp tối ưu bộ nhớ RAM tránh tạo rác liên tục
    button_labels = ['START', 'WEAPON', 'SCORE', 'EXIT']
    buttons = [
        (pygame.Rect((WIDTH - 220) // 2, 220 + 80 * i, 220, 60), label) 
        for i, label in enumerate(button_labels)
    ]
    
    while True:
        # Xử lý cuộn lặp liên tục background chính sang bên trái tạo cảm giác động mượt mà
        ...
        # Hệ thống hứng Event đóng cửa sổ hoặc nhấn chuột
        for event in pygame.event.get():
            if event.type == pygame.QUIT: 
                pygame.quit()
                sys.exit()
